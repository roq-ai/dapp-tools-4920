...
export function TaskListPage(props: TaskListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TaskInterface>>(
    () => `/tasks?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'status', header: 'Status', accessorKey: 'status' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.deadline ? format(parseISO(record?.deadline as unknown as string), 'dd-MM-yyyy') : '',
    },
