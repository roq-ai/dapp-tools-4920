...
import { getOrganizationOwners } from 'apiSdk/organization-owners';
...
import { OrganizationOwnerListPage as ShowCaseList } from 'pages/organization-owners';

function DashboardPage() {
  const { session } = useSession();

  const organizationOwnerStat = useSWR<PaginatedInterface<OrganizationOwnerInterface>>(
    () => `/organization-owners`,
    () => getOrganizationOwners({ limit: 1, offset: 0 }),
  );

  const developerStat = useSWR<PaginatedInterface<DeveloperInterface>>(
    () => `/developers`,
    () => getDevelopers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: organizationOwnerStat?.data?.totalCount,
      isLoading: organizationOwnerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Developers',
      result: developerStat?.data?.totalCount,
      isLoading: developerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Projects',
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Organization Owners
                </Text>
                <NextLink href={`organization-owners`} passHref legacyBehavior>
                  <Button
