...
import { getDevelopers, deleteDeveloperById } from 'apiSdk/developers';
  ...
  const fetcher = useCallback(
    async () =>
      getDevelopers({
        ...
        order: params.order,
        searchTermKeys: ['name.contains', 'specialization.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DeveloperInterface>>(
    () => `/developers?params=${JSON.stringify(params)}`,
    fetcher,
  );
